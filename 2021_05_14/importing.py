...
def selectFile(title = "Choose filename to save", filetypes = [("csv file (.csv)",".csv"),("numpy file (.npz)",".npz")], save = True):    
    root = Tk(); root.iconify() 
    if save:
        ...
        loc = fd.askopenfilename(title= title,\
                                parent=root,\
                                filetypes= filetypes)
    root.destroy()    
    return loc
